...
(function () {
  ...
  const ADD = {
    uk: {
      ...
      fracLabels: { 0: '', 1: '¼', 2: '½' },
      fracNone: 'без дробу',
      buildTarget: (str) => `Налий рівно <span class="price">${str}</span> з доступних пляшок!`,
    ...
    ru: {
      ...
      fracLabels: { 0: '', 1: '¼', 2: '½' },
      fracNone: 'без дроби',
      buildTarget: (str) => `Налей ровно <span class="price">${str}</span> из доступных бутылок!`,
    ...
    es: {
      ...
      buildTarget: (str) => `¡Sirve exactamente <span class="price">${str}</span> con las botellas disponibles!`,
  ...
  function quartersToParts(q) { return { whole: Math.floor(q / 4), fracIdx: q % 4 }; }
  ...
  function renderCountTask(withOptions) {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (window.taskMemory.accept('volumes-count-' + (withOptions ? 'O' : 'I'), target, 5)) break;
    }
    ...
    $('#taskText').innerHTML = `${scenario}<br><b>${t.volumeHowMuch}</b><br><small style="opacity:0.8;font-size:0.85em;">${hint}</small>`;
    ...
    if (withOptions) {
      ...
      controls.innerHTML = options.map(q =>
        `<button class="btn option" data-val="${q}">${formatVolume(q)}</button>`
      ).join('');
      ...
      document.querySelectorAll('#volFracBtns .frac-btn').forEach(b => {
        b.addEventListener('click', () => {
          ...
          document.querySelectorAll('#volFracBtns .frac-btn').forEach(x =>
            x.classList.toggle('active', x === b));
        });
      });
      whole.addEventListener('keydown', (e) => { if (e.key === 'Enter') $('#checkVolBtn').click(); });
      $('#checkVolBtn').onclick = () => {
  ...
  function renderBuildTask() {
    ...
    $('#taskText').innerHTML = `${scenario}<br>${t.buildTarget(formatVolume(target))}`;
    ...
    area.innerHTML = `
      ...
      <div class="pay-zone tray-zone">
        ...
        <div class="tray weight-tray" id="vTray"><div class="tray-empty">${t.trayEmpty || ''}</div></div>
      </div>
    `;
    ...
    controls.innerHTML = `
      ${showSum ? `<div class="current-sum">${t.currentSum} <span class="val" id="vCurrentSum">0 ${t.L}</span></div>` : ''}
  ...
  function renderTray(animateLast) {
    ...
    sorted.forEach(s => {
      const wrap = document.createElement('div');
      wrap.innerHTML = volumeHtml(s, animateLast && s.idx === state.selected.length - 1 ? 'drop-in' : '');
      const el = wrap.firstChild;
  ...
  window.Games.volumes = {
    id: 'volumes',
    icon: '🥤',
    getName: (lang) => (window.I18N && window.I18N[lang] && window.I18N[lang].gameVolumes) || 'Volumes',
    newRound,
    _test: { PIECES, VDIFF, isAllowedQ, randAllowedQ, makeVolumeMix, formatVolume, quartersToParts, generateOptions },
  };
})();
